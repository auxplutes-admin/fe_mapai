...
const SessionPage: React.FC = () => {
  ...
  // If sessionId and region are provided via URL, open chat directly with same UI
  useEffect(() => {
    const run = async () => {
      try {
        if (!sessionId || !regionId || !regionName) return;
        // persist session id for continuity across app
        localStorage.setItem('chat_session_id', sessionId);

        // Ensure region exists; fallback to opening with provided ids
        try {
          const regions = await getAllRegions();
          const match = regions?.find?.((r: any) => r.region_id === regionId);
          const title = match?.province_name
            ? `${regionName} - ${match.province_name}`
            : regionName;
          const summary = match?.summary;
          setDetail({ title, selectedProvince: match?.province_name || regionName, id: regionId, summary });
        } catch {
          // If regions fetch fails, still open chat with minimal info
          setDetail({ title: regionName, selectedProvince: regionName, id: regionId });
        }
    ...
    run();
    // Only run when URL parts change
  }, [sessionId, regionId, regionName]);

  return (
    <div className="flex h-screen">
      <div className="relative w-full map-root">
        {/* Top-left hint badge (always visible, sits below +/-) */}
        <div className="map-hint mt-10">Choose an area</div>
